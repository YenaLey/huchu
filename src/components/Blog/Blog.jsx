...
function Blog() {
    ...
    let [list, list변경] = useState([
        ...
        ['세계의 귀여운 고양이 종류', '이예나', "2024.03.03", "브리티시쇼트헤어, 아메리칸쇼트헤어, 라가머핀, 터키시앙고라, 러시안블루, 페르시안고양이, 노르웨이숲, 시베리안숲, 먼치킨, 걈, 버만, 버미즈, 뱅갈 등 넘나 많다. 이 중에서 러시안 블루가 가장 귀염뽀짝하다고 할 수 있다.", 0, 0],

        ['고양이가 절대 먹으면 안 되는 음식', '이예나', "2024.03.04", "날생선, 초콜릿, 강아지 사료, 유제품, 카페인, 견과류 등등등 굉장히 많다. 고양이가 맞나 싶다.", 0, 0],

        ['고양이가 우는 이유', '이예나', "2024.03.05", "야행성 동물인 고양이는 낮에는 내내 잠을 자고, 밤이 되면 활동을 시작한다. 우다다다를 시전하며 신난 고양이는 낮에 비축한 에너지를 폭발적으로 발산한다.", 0, 0]]);
        ...
        <div className="blog-container">
            ...
            <div className="left-container">
                <div className="form">
                    ...
                    <div className="w-100">
                        <button onClick={() => {
                            if (제목입력 == '' || 작성자입력 == '' || 작성일자입력 == '' || 내용입력 == '') {
                                alert("내용을 입력하세요");
                            } else {
                                const copylist = [...list];
                                copylist.unshift([제목입력, 작성자입력, 작성일자입력, 내용입력, 0, 0])
                                list변경(copylist);
                            }
                        }}>업로드</button>
                    </div>
            ...
            <div className="right-container">
                {
                    list.map(function (element, index) {
                        return (
                            <List
                                list={list} list변경={list변경} setModal={setModal} setModalIndex={setModalIndex}
                                index={index}>
                            </List>
                        )
                    })
            ...
            {
                modal == true ? <Modal list={list} modalIndex={modalIndex} setModal={setModal}></Modal> : null
            }
        </div>
    );
}
